...
void ARDriver::Reset()
{
  ///Mise à jour des structures à chaque image
  mBatiment.Reset();
  mnCounter = 0;
}
...
void ARDriver::Render(Image<Rgb<byte> > &imFrame, SE3<> se3CfromW)
{
  ...
  mBatiment.DrawStuff(se3CfromW.inverse().get_translation());
...
///Fonctions de zoom avant et arrière

void ARDriver::ZoomUp()
{
    mBatiment.ZoomUp();
}
void ARDriver::ZoomDown()
{
    mBatiment.ZoomDown();
}

///Fonctions de rotation
void ARDriver::Rotate()
{
    mBatiment.Rotate();
}
